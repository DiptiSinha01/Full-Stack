...
export default function ReflectionPrompt({ reflections, setReflections }) {
  ...
  useEffect(() => {
    // Only proceed if reflections data is loaded (not undefined)
    if (reflections === undefined) return;
    
    const hasAnsweredToday = reflections?.[today];
    const alreadyShownToday = sessionStorage.getItem("reflection-shown-today") === "true";
    
    // Show prompt if they haven't answered today and it's not already shown in this session
    if (!hasAnsweredToday && !alreadyShownToday) {
